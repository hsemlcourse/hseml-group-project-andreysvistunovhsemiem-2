...
Будет заполнено в CP3 (FastAPI `/predict`, контейнер из текущего `Dockerfile`).

## 8. Заключение и выводы

- На CP1 показал, что в данных есть сигнал и линейный baseline даёт ROC-AUC ≈ 0.763.
- На CP2 тюнинг + калибровка + порог поднимают XGBoost до ROC-AUC ≈ 0.776 на test
  (CV-подтверждённо), а recall на бизнес-пороге растёт без существенной потери ROC-AUC.
- PCA не дал улучшения для линейных моделей — основные взаимодействия в данных нелинейны;
  ансамбли деревьев — естественный выбор.
- Артефакты CP2 (`tuned_*.joblib`) пригодны для деплоя в CP3 без переобучения.
